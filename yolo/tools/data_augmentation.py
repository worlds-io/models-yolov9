...
class RandomPad:
    ...
    def __call__(self, image, boxes):
        if torch.rand(1) < self.prob:
            ...
            # paste the original image into the new, larger image
            padded_image.paste(image, (left, top))

            # scale the XY values from [0, 1] to [0, original_size], then add the new padding
            boxes[:, [1, 3]] = boxes[:, [1, 3]] * original_width + left
            boxes[:, [2, 4]] = boxes[:, [2, 4]] * original_height + top

            # scale the XY values back to [0, 1], relative to the size of the new, larger image
...
class Colorspace:
    ...
    def __call__(self, image, boxes, hue=10, saturation=1.5, exposure=1.5):
        if torch.rand(1) < self.prob:
            ...
            dexposure   = np.random.uniform(1/exposure,   exposure)
